//* This file is part of the MOOSE frameworkCustomIsotropicElasticityTensor
...
registerMooseObject("TensorMechanicsApp", CustomIsotropicElasticityTensor);
registerMooseObject("TensorMechanicsApp", ADCustomIsotropicElasticityTensor);
...
InputParameters
CustomIsotropicElasticityTensorTempl<is_ad, T>::validParams()
{
  ...
  params.addCoupledVar("lambda", "Lame's first constant for the material.");
  ...
  params.declareControllable("bulk_modulus poissons_ratio shear_modulus youngs_modulus");
  return params;
}

template <bool is_ad, typename T>
CustomIsotropicElasticityTensorTempl<is_ad, T>::CustomIsotropicElasticityTensorTempl(
  ...
  : ComputeElasticityTensorBaseTempl<is_ad, T>(parameters),
    ...
    _lambda(this->template coupledScalarValue("lambda")),
...
template <bool is_ad, typename T>
void
CustomIsotropicElasticityTensorTempl<is_ad, T>::residualSetup()
{
  ...
  {
    iso_const[0] = _lambda[0];
    iso_const[1] = _shear_modulus;
    elas_mod =
        (_shear_modulus * (3 * _lambda[0] + 2 * _shear_modulus)) / (_lambda[0] + _shear_modulus);
    poiss_rat = _lambda[0] / (2 * (_lambda[0] + _shear_modulus));
    _effective_stiffness_local =
  ...
  {
    iso_const[0] = _lambda[0];
    iso_const[1] = 3.0 * (_bulk_modulus - _lambda[0]) / 2.0;
    elas_mod =
        (9 * _bulk_modulus * (_bulk_modulus - _lambda[0])) / (3 * _bulk_modulus - _lambda[0]);
    poiss_rat = (_lambda[0]) / ((3 * _bulk_modulus - _lambda[0]));
    _effective_stiffness_local =
        std::max(std::sqrt((elas_mod * (1 - poiss_rat)) / ((1 + poiss_rat) * (1 - 2 * poiss_rat))),
                 std::sqrt(elas_mod / (2 * (1 + poiss_rat))));
  }
  ...
  {
    iso_const[0] = _lambda[0];
    iso_const[1] = _lambda[0] * (1.0 - 2.0 * _poissons_ratio) / (2.0 * _poissons_ratio);
    elas_mod = (_lambda[0] * (1 + _poissons_ratio) * (1 - 2 * _poissons_ratio)) / (_poissons_ratio);
    poiss_rat = (_poissons_ratio);
    _effective_stiffness_local =
        std::max(std::sqrt((elas_mod * (1 - poiss_rat)) / ((1 + poiss_rat) * (1 - 2 * poiss_rat))),
                 std::sqrt(elas_mod / (2 * (1 + poiss_rat))));
  ...
  {
    iso_const[0] = _lambda[0];
    iso_const[1] = (_youngs_modulus - 3.0 * _lambda[0] +
                    std::sqrt(_youngs_modulus * _youngs_modulus + 9.0 * _lambda[0] * _lambda[0] +
                              2.0 * _youngs_modulus * _lambda[0])) /
                   4.0;
    elas_mod = (_youngs_modulus);
    poiss_rat =
        (2 * _lambda[0]) / (_youngs_modulus + _lambda[0] +
                            std::sqrt(std::pow(_youngs_modulus, 2) + 9 * std::pow(_lambda[0], 2) +
                                      2 * _youngs_modulus * _lambda[0]));
    _effective_stiffness_local =
        std::max(std::sqrt((elas_mod * (1 - poiss_rat)) / ((1 + poiss_rat) * (1 - 2 * poiss_rat))),
                 std::sqrt(elas_mod / (2 * (1 + poiss_rat))));
  }
  else
    mooseError("Incorrect combination of elastic properties in CustomIsotropicElasticityTensor.");
...
template <bool is_ad, typename T>
void
CustomIsotropicElasticityTensorTempl<is_ad, T>::computeQpElasticityTensor()
{
...
template class CustomIsotropicElasticityTensorTempl<false, RankFourTensor>;
template class CustomIsotropicElasticityTensorTempl<true, RankFourTensor>;
template class CustomIsotropicElasticityTensorTempl<false, SymmetricRankFourTensor>;
template class CustomIsotropicElasticityTensorTempl<true, SymmetricRankFourTensor>;
